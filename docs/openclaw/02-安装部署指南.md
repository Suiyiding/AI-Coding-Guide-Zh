...
> **v2026.5.27 安装基线**：官方 README 当前写法是 Node 24 recommended，Node 22.19+ supported。新装优先走 `openclaw onboard`，升级和回滚按 release channel 选择 stable / beta / dev；教程里的固定版本号只用于复现问题，不建议长期锁死。v2026.5.28 目前是 beta，v2026.5.29-alpha.1 仅作为 tag 出现，不作为本教程默认稳定基线。
...
- Gateway 启动路径有缓存和延迟加载优化，升级后启动变快是正常现象；启动失败时看 `openclaw doctor`、Gateway 日志和 v2026.5.27 release verification。
- Docker runtime workspace templates、Alpine 安装、Windows Scheduled Tasks 和稳定 channel 选择在 v2026.5.26 / v2026.5.27 有加固；生产部署不要跳过发布包 integrity / shrinkwrap 校验。
