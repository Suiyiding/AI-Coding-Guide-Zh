...
> **v2026.5.27 提示**：如果你只输入 `openclaw`，且本机还没有 authored config，新版会进入 classic onboarding。教学里仍建议写完整命令，避免读者分不清“只启动 CLI”和“安装后台服务”的区别。快速开始后的排障优先看 Control UI Activity、transcript、Gateway 日志和 `openclaw doctor`，不要只凭聊天窗口是否回复来判断是否安装成功。
