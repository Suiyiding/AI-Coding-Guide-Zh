...
配合多 Agent 路由，让不同联系人的消息自动路由到对应 Agent。在顶层 `bindings` 配置中设置 `channel`、`contact` 和 `agent` 的映射关系即可（bindings 是顶层配置，不嵌套在 channels 内部）。详见 [08. 多 Agent 路由](08-多Agent协作指南.md)。
